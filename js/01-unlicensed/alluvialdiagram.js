document.addEventListener("DOMContentLoaded", function() {
    ...
    // Dimensions - Increased top margin for Axis Labels
    ...
    // Add Axis Labels (Alluvial Style)
    svg.append("text")
        .attr("x", 0)
        .attr("y", -15)
    ...
    svg.append("text")
        ...
        .attr("font-size", "14px")
        .attr("fill", "#333")
        .text("Enforcement Outcome");

    d3.csv("data/police_enforcement_2024_fines.csv").then(function(data) {
        
        // 1. Filter for Unlicensed Driving
        const metricData = data.filter(d => d.METRIC === "unlicensed_driving");
        ...
        categories.forEach((cat, i) => {
            outcomes.forEach(outcome => {
                ...
                if (value > 0) {
                    links.push({ source: i, target: outcomeIndices[outcome], value: value });
                }
            });
        });

        // 4. Setup Sankey (Alluvial Configuration)
        const sankey = d3.sankey()
            .nodeWidth(15)       // Thinner nodes act as axes
            .nodePadding(20)     // Tighter packing
            .extent([[1, 1], [width - 1, height - 6]])
            .nodeSort((a, b) => b.value - a.value); // Sort by size (Alluvial characteristic)
        ...
        linkPath.on("mouseover", function(event, d) {
            ...
            const totalSource = d.source.value;
            const percent = ((d.value / totalSource) * 100).toFixed(1);

            tooltip.transition().duration(200).style("opacity", .9);
            tooltip.html(`
                <strong>${d.source.name} → ${d.target.name}</strong><br/>
                ${d.value.toLocaleString()} instances<br/>
                <span style="color:red">(${percent}% of ${d.source.name} volume)</span>
            `)
        ...
        const node = svg.append("g")
            ...
            .attr("fill", d => {
                if(d.name === "Camera-based") return "#1976D2";
                if(d.name === "Police-issued") return "#D32F2F";
                return "#555"; 
            })
            .attr("opacity", 0.9);
        ...
        node.on("mouseover", function(event, d) {
            d3.selectAll("path").transition().duration(200)
                .attr("stroke-opacity", l => (l.source === d || l.target === d) ? 0.8 : 0.05);
